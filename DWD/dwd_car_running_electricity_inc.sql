...
insert overwrite table car_data.dwd_car_running_electricity_inc partition(dt)
select
    `vin`,
    `velocity`,
    `mileage`,
...
-- 每日装载
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table car_data.dwd_car_running_electricity_inc partition(dt='2023-05-03')
select
    `vin`,
    `velocity`,
    `mileage`,
